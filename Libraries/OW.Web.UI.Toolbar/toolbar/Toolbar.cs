...
namespace OW.Web.UI.Toolbar
{
	...
	public class Toolbar : WebControl, INamingContainer
	{
		...
		/// Inits the control.
		/// </summary>
		public Toolbar() : base(HtmlTextWriterTag.Table)
		{
		...
		protected void RenderItemCell(ToolbarItem item, HtmlTextWriter writer) 
		{
			...
			cell.Attributes.Add("onmouseover", item.ItemCellMouseOver);
			cell.Attributes.Add("onmouseout", item.ItemCellMouseOut);
			...
			if (item is ToolbarSeparator) 
			{
				...
				//No mouse events for separator
				cell.Attributes.Add("onmouseover", "this.className=null;");
				cell.Attributes.Add("onmouseout", "this.className=null;");
				//No postback event for separator
				cell.Attributes.Add("onclick","javascript:if(true)return false;");
			...
			else
			{
				...
				//-------------------------------------------------------
				if(item is ToolbarImage)
				{
					string onclick = string.Empty;
					string postback = string.Empty;

					//If item is not blank add postback event
					if(item.ItemId != "Blank" && item.ItemId != "None")
					{
						//Clear cellover
						string cellClassName = " document.getElementById('CELL_" + ((ToolbarImage)item).ClientID + "').className = null; ";
						//Disabled cell
						string cellDisabled = " document.getElementById('CELL_" + ((ToolbarImage)item).ClientID + "').disabled = true; ";
						//Set postback
						postback = cellClassName + cellDisabled + Page.GetPostBackEventReference(((ToolbarImage)item));
					}

					//Add client script if exists
					if (((ToolbarImage)item).ClientScript.Trim() != string.Empty) onclick += ((ToolbarImage)item).ClientScript;
					...
					if (((ToolbarImage)item).CauseValidation)
						onclick += "if (typeof(Page_ClientValidate) == 'function'){ if(Page_ClientValidate()){" + postback + "}} else {" + postback + "}";
					else 
						onclick += postback;
					...
					//ID da celula
					cell.Attributes.Add("id","CELL_" + item.ClientID);
				}
				//-------------------------------------------------------
